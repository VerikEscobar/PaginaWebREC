<?php 
$title       = "Leyes | Registro del Estado Civil";
...
if (!empty($_GET['buscar']) || isset($_GET['btn-buscar'])) {
    if (empty($_GET['buscar'])) {
        echo "<script>location.href ='" . url() . "leyes';</script>";
        exit;
    }
    $b = limpia_url($_GET['buscar']);
    header("location:" . url() . "leyes/buscar/$b");
}
if (!empty($_GET['desde']) && !empty($_GET['hasta'])) {
    $d = url_amigable(fechaLatinaURL($_GET['desde']));
    $h = url_amigable(fechaLatinaURL($_GET['hasta']));
    header("location:" . url() . "leyes/desde/$d/hasta/$h");
}
?>
    ...
    <section class="blog-page-section sec-padding">
        <div class="container">
            <div class="row">
                        ...
                        <div class="search-widget">
                            <form id="formBuscar">
                                <div class="contenedor">
                                    <input type="text" id="buscar" class="form-control" name="buscar" placeholder="Buscar ley" autocomplete="off">
                                    <div class="dytroit">
                ...
                traerDocumentos(1,"leyes", $pag, 12, $buscar, $desde, $hasta);
                ?>
                </div>
            </div>
        </div>
