<?php 
$title       = "Convenios | Registro del Estado Civil";
...
if (!empty($_GET['buscar']) || isset($_GET['btn-buscar'])) {
    if (empty($_GET['buscar'])) {
        echo "<script>location.href ='" . url() . "convenios';</script>";
        exit;
    }
    $b = limpia_url($_GET['buscar']);
    header("location:" . url() . "convenios/buscar/$b");
}
if (!empty($_GET['desde']) && !empty($_GET['hasta'])) {
    $d = url_amigable(fechaLatinaURL($_GET['desde']));
    $h = url_amigable(fechaLatinaURL($_GET['hasta']));
    header("location:" . url() . "convenios/desde/$d/hasta/$h");
}
?>
    ...
    <section class="blog-page-section sec-padding">
        <div class="container">
            <div class="row">
                ...
                traerDocumentos(6,"convenios", $pag, 12, $buscar, $desde, $hasta);
                ?>
                </div>
            </div>
        </div>
