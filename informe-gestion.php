<?php 
$title       = "Informes de Gestión | Registro del Estado Civil";
...
if (!empty($_GET['buscar']) || isset($_GET['btn-buscar'])) {
    if (empty($_GET['buscar'])) {
        echo "<script>location.href ='" . url() . "informes-de-Gestion';</script>";
        exit;
    }
    $b = limpia_url($_GET['buscar']);
    header("location:" . url() . "informes-de-Gestion/buscar/$b");
}
if (!empty($_GET['desde']) && !empty($_GET['hasta'])) {
    $d = url_amigable(fechaLatinaURL($_GET['desde']));
    $h = url_amigable(fechaLatinaURL($_GET['hasta']));
    header("location:" . url() . "informes-de-Gestion/desde/$d/hasta/$h");
}
?>
    ...
    <section class="blog-page-section sec-padding">
        <div class="container">
            <div class="row">
                        ...
                        <div class="search-widget">
                            <form id="formBuscar">
                                <div class="contenedor">
                                    <input type="text" id="buscar" class="form-control" name="buscar" placeholder="Buscar informe" autocomplete="off">
                                    <div class="dytroit">
                ...
                traerDocumentos(9,"Informes de Gestion", $pag, 12, $buscar, $desde, $hasta);
                ?>
                </div>
            </div>
        </div>
