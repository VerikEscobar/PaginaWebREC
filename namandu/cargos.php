...
<body class="<?php include 'menu-class.php';?> fixed-layout">
                        ...
                        <div id="toolbar">
                            <div class="form-inline" role="form">
                                <div class="form-group">
                                    <button type="button" class="btn btn-primary" id="agregar" data-toggle="modal" data-target="#modal_principal">Agregar Cargo</button>
                                </div>
                            </div>
                        </div>
                        <table id="tabla" data-url="inc/cargos-data?q=ver" data-toolbar="#toolbar" data-show-export="true" data-search="true" data-show-refresh="true" data-show-toggle="true" data-show-columns="true" data-search-align="right" data-buttons-align="right" data-toolbar-align="left" data-pagination="false" data-classes="table table-hover table-condensed" data-striped="true" data-icons="icons" data-show-fullscreen="true"></table>
                        <!-- MODA PRINCIPAL -->
                                        ...
                                        <div class="modal-body">
                                            <div class="row">
                                                <div class="col-md-9 form-group">
                                                    ...
                                                    <input class="form-control input-sm" type="text" name="cargo" id="cargo" autocomplete="off" required>
                                                </div>
    ...
    window.accionesFila = {
        'click .cambiar-estado': function(e, value, row, index) {
            ...
            swal({
                ...
                text: `¿Actualizar estado del cargo a '${nextStatus}'?`,
                type: "warning",
                ...
                $.ajax({
                    url: 'inc/cargos-data?q=cambiar-estado',
                    type: 'post',
                    data: { id: row.id_directivo_cargo, estado: status },
                    beforeSend: function() {
                        NProgress.start();
                    },
                    success: function(datos, textStatus, jQxhr) {
                        ...
                        if (n == -1) {
                            ...
                            $('#tabla').bootstrapTable('refresh', { url: 'inc/cargos-data?q=ver' });
                        } else {
                            swal.close();
        ...
        'click .editar': function(e, value, row, index) {
            $('#modalLabel').html('Editar Cargo');
            $('#formulario').attr('action', 'editar');
            $('#eliminar').show();
            $('#modal_principal').modal('show');
            $("#mensaje, #mensaje_modal").html("");

            $("#hidden_id").val(row.id_autoridad_cargo);
            $("#cargo").val(row.cargo);
            $("#orden").val(row.orden);

            //ELIMINAR
            $('#eliminar').click(function() {
                var nombre = row.cargo;
                swal({
                    ...
                    $.ajax({
                        ...
                        url: 'inc/cargos-data',
                        cache: false,
                        data: { q: 'eliminar', id: $("#hidden_id").val(), nombre: row.cargo },
                        beforeSend: function() {
    ...
    $("#formulario").submit(function(e) {
        ...
        $.ajax({
            url: 'inc/cargos-data?q=' + $(this).attr("action"),
            dataType: 'html',
            type: 'post',
