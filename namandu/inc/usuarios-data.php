<?php
	...
	switch ($q){
		...
		case 'ver':
			...
			$db->setQuery("SELECT u.id,u.email,u.password,u.username,u.nombre_apellido,u.departamento,u.cargo,u.ci,u.telefono,u.direccion,u.id_sucursal,CONCAT(s.nombre_empresa,' - ',s.sucursal) AS sucursal, u.foto,u.status,u.roles_mask,u.registered,u.last_login,u.force_logout,u.id_rol,r.rol FROM users u LEFT JOIN sucursales s ON s.id_sucursal=u.id_sucursal LEFT JOIN roles r ON r.id_rol=u.id_rol ORDER BY u.id_sucursal, username");
		...
		case 'cargar':
			...
			try {
				...
				$db->setQuery("UPDATE users SET nombre_apellido='$nombre', departamento='$dpto', cargo='$cargo', ci='$ci', telefono='$telefono', direccion='$direccion', id_sucursal='$sucursal', foto='dist/images/users/nobody.png', status=$status, id_rol='$id_rol' WHERE id=$userId");
		...
		case 'editar':
			...
			$db->setQuery("UPDATE users SET ci='$ci', nombre_apellido='$nombre', id_sucursal='$sucursal', telefono='$telefono', direccion='$direccion', email='$email', departamento='$dpto', cargo='$cargo', status='$estado', id_rol=$id_rol WHERE id = '$id_usuario'");
