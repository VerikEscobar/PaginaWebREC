...
<body class="<?php include 'menu-class.php';?> fixed-layout">
                        ...
                        <div id="toolbar">
                            <div class='form-inline' role='form'>
                                <div class='form-group'>
                                    <button type='button' class='btn btn-primary' id='agregar' data-toggle='modal' data-target='#modal_principal'>Agregar Formulario</button>
                                </div>
                            </div>
                        </div>
                        <table id="tabla" data-url="inc/formulario-data?q=ver" data-toolbar="#toolbar" data-show-export="true" data-search="true" data-show-refresh="true" data-show-toggle="true" data-show-columns="true" data-search-align="right" data-buttons-align="right" data-toolbar-align="left" data-pagination="true" data-side-pagination="server" data-classes="table table-hover table-condensed" data-page-list="[100, 150, 200, 300, 400, All]" data-striped="true" data-icons="icons" data-show-fullscreen="true"></table>
    ...
    window.filaEditar = {
        'click .cambiar-estado': function(e, value, row, index) {
            ...
            swal({
                ...
                text: `¿Actualizar estado del Formulario a '${nextStatus}'?`,
                type: "warning",
                ...
                $.ajax({
                    url: 'inc/formulario-data?q=cambiar-estado',
                    type: 'post',
                    data: { id: row.id_formulario, estado: status },
                    beforeSend: function() {
                        NProgress.start();
                    },
                    success: function(datos, textStatus, jQxhr) {
                        ...
                        if (n == -1) {
                            ...
                            $('#tabla').bootstrapTable('refresh', { url: 'inc/formulario-data?q=ver' });
                        } else {
                            swal.close();
    ...
    $("#tabla").bootstrapTable({
        ...
        columns: [
            [
                { field: 'id_formulario', visible: false },
                ...
                { field: 'fecha_formulario', align: 'center', valign: 'middle', title: 'Fecha del Formulario', sortable: false, width: 150, visible: false},
                { field: 'fecha', align: 'center', valign: 'middle', title: 'Fecha de Creación', sortable: false, width: 150, visible: true },
    ...
    function editarFila(e, value, row, index) {
        ...
        $('#modalLabel').html('Editar Formulario');
        $('#eliminar').show();
        $("#dropurl").val('editar');
        $("#contenDrop").load("./dropzone-formularios.html");
        $('#modal_principal').modal('show');
        $("#hidden_id").val(row.id_formulario);
        $("#titulo").val(row.titulo);
        $("#fecha").val(row.fecha_formulario);
        $("#descripcion").val(row.descripcion);
        $.ajax({
            url: 'inc/formulario-data',
            type: 'post',
            data: { q: 'leer_fotos', id: row.id_formulario },
            dataType: 'json',
    ...
    $('#agregar').click(function() {
        $('#modalLabel').html('Agregar Formulario');
    ...
    $('#eliminar').click(function() {
        ...
        swal({
            title: "Eliminar",
            text: "¿Desea eliminar este Formulario?",
            type: "warning",
            showCancelButton: true,
            confirmButtonColor: "var(--primary)",
