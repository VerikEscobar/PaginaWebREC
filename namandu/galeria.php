...
<body class="<?php include 'menu-class.php';?> fixed-layout">
                        ...
                        <div id="toolbar">
                            <div class='form-inline' role='form'>
                                <div class='form-group'>
                                    <button type='button' class='btn btn-primary' id='agregar' data-toggle='modal' data-target='#modal_principal'>Agregar Galeria</button>
                                </div>
                            </div>
                        </div>
                        <table id="tabla" data-url="inc/galerias-data?q=ver" data-toolbar="#toolbar" data-show-export="true" data-search="true" data-show-refresh="true" data-show-toggle="true" data-show-columns="true" data-search-align="right" data-buttons-align="right" data-toolbar-align="left" data-pagination="true" data-side-pagination="server" data-classes="table table-hover table-condensed" data-page-list="[100, 150, 200, 300, 400, All]" data-striped="true" data-icons="icons" data-show-fullscreen="true"></table>
                                            ...
                                            <div class="form-row">
                                                <div class="col-md-12">
                                                    <div class="row">
                                                        ...
                                                        <div class="form-group col-md-12 col-sm-12">
                                                            <label for="copete">Video</label>
                                                            <input class="form-control input-sm" type="text" name="video" id="video" autocomplete="off">
                                                        </div>
    ...
    window.filaEditar = {
        'click .cambiar-estado': function(e, value, row, index) {
            ...
            swal({
                ...
                text: `¿Actualizar estado de la galeria a '${nextStatus}'?`,
                type: "warning",
                ...
                $.ajax({
                    url: 'inc/galerias-data?q=cambiar-estado',
                    type: 'post',
                    data: { id: row.id_galeria, estado: status },
                    beforeSend: function() {
                        NProgress.start();
                    },
                    success: function(datos, textStatus, jQxhr) {
                        ...
                        if (n == -1) {
                            ...
                            $('#tabla').bootstrapTable('refresh', { url: 'inc/galerias-data?q=ver' });
                        } else {
                            swal.close();
    ...
    $("#tabla").bootstrapTable({
        ...
        columns: [
            [
                { field: 'id_galeria', visible: false },
                { field: 'titulo', align: 'left', valign: 'middle', title: 'Titulo', sortable: true, visible: true },
                { field: 'video', align: 'left', valign: 'middle', title: 'Url del Video', sortable: true, visible: false },
                { field: 'copete', align: 'left', valign: 'middle', title: 'Descripción', sortable: true, cellStyle: truncarColumna },
                { field: 'foto', align: 'center', valign: 'middle', title: 'Foto', sortable: false, formatter: fotos },
                { field: 'fecha', align: 'center', valign: 'middle', title: 'Fecha', sortable: false },
                ...
                { field: 'fecha_galeria', visible: false },
                { field: 'destacado', visible: false }
            ]
        ]
    });
    ...
    function editarFila(e, value, row, index) {
        ...
        $('#modalLabel').html('Editar Galeria');
        $('#eliminar').show();
        $("#dropurl").val('editar');
        $("#contenDrop").load("./dropzone-galerias.html");
        $('#modal_principal').modal('show');
        $("#hidden_id").val(row.id_galeria);
        $('#editor').summernote('code', row.descripcion);
        $("#titulo").val(row.titulo);
        $("#copete").val(row.copete);
        $("#fecha").val(row.fecha_galeria);
        ...
        $.ajax({
            url: 'inc/galerias-data',
            type: 'post',
            data: { q: 'leer_fotos', id: row.id_galeria },
            dataType: 'json',
    ...
    $('#agregar').click(function() {
        $('#modalLabel').html('Agregar Galeria');
    ...
    $('#eliminar').click(function() {
        ...
        swal({
            title: "Eliminar",
            text: "¿Desea eliminar la Galeria: " + nombre + "?",
            type: "warning",
            showCancelButton: true,
            confirmButtonColor: "var(--primary)",
