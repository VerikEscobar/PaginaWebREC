...
<body class="<?php include 'menu-class.php';?> fixed-layout">
                        ...
                        <div id="toolbar">
                            <div class="form-inline" role="form">
                                <div class="form-group">
                                    <button type="button" class="btn btn-primary" id="agregar" data-toggle="modal" data-target="#modal_principal">Agregar Red Social</button>
                                </div>
                            </div>
                        </div>
                        <table id="tabla" data-url="inc/sociales-data?q=ver" data-toolbar="#toolbar" data-show-export="true" data-search="true" data-show-refresh="true" data-show-toggle="true" data-show-columns="true" data-search-align="right" data-buttons-align="right" data-toolbar-align="left" data-pagination="false" data-classes="table table-hover table-condensed" data-striped="true" data-icons="icons" data-show-fullscreen="true"></table>
                        <!-- MODA PRINCIPAL -->
                                        ...
                                        <div class="modal-body">
                                            <div class="row">
                                                ...
                                                <div class="col-md-6 form-group">
                                                    ...
                                                    <input class="form-control input-sm" type="text" name="icono" id="icono" autocomplete="off" required>
                                                </div>
    ...
    window.accionesFila = {
        'click .cambiar-estado': function(e, value, row, index) {
            ...
            swal({
                ...
                $.ajax({
                    url: 'inc/sociales-data?q=cambiar-estado',
                    type: 'post',
                    data: { id: row.id_red_social, estado: status },
                    beforeSend: function() {
                        NProgress.start();
                    },
                    success: function(datos, textStatus, jQxhr) {
                        ...
                        if (n == -1) {
                            ...
                            $('#tabla').bootstrapTable('refresh', { url: 'inc/sociales-data?q=ver' });
                        } else {
                            swal.close();
        ...
        'click .editar': function(e, value, row, index) {
            $('#modalLabel').html('Editar Red Social');
            $('#formulario').attr('action', 'editar');
            $('#eliminar').show();
            $('#modal_principal').modal('show');
            $("#mensaje, #mensaje_modal").html("");

            $("#hidden_id").val(row.id_red_social);
    ...
    $("#tabla").bootstrapTable({
        ...
        sortName: "r.id_red_social",
        sortOrder: 'asc',
        height: $(window).height() - 90,
        pageSize: Math.floor(($(window).height() - 90) / 50),
        columns: [
            [
                { field: 'id_red_social', align: 'center', valign: 'middle', title: 'ID', sortable: true, visible: false },
                ...
                { field: 'icono', align: 'left', valign: 'middle', title: 'Icono', sortable: true, visible: false },
    ...
    $('#agregar').click(function() {
        $('#modalLabel').html('Agregar Red Social');
    ...
    $("#formulario").submit(function(e) {
        ...
        $.ajax({
            url: 'inc/sociales-data?q=' + $(this).attr("action"),
            dataType: 'html',
            type: 'post',
