...
<body class="<?php include 'menu-class.php';?> fixed-layout">
                        ...
                        <div id="toolbar">
                            <div class="form-inline" role="form">
                                <div class="form-group">
                                    <button type="button" class="btn btn-primary" id="agregar" data-toggle="modal" data-target="#modal_principal">Agregar Boton</button>
                                </div>
                            </div>
                        </div>
                        <table id="tabla" data-url="inc/botones-data?q=ver" data-toolbar="#toolbar" data-show-export="true" data-search="true" data-show-refresh="true" data-show-toggle="true" data-show-columns="true" data-search-align="right" data-buttons-align="right" data-toolbar-align="left" data-pagination="false" data-classes="table table-hover table-condensed" data-striped="true" data-icons="icons" data-show-fullscreen="true"></table>
                        <!-- MODA PRINCIPAL -->
    ...
    window.accionesFila = {
        'click .cambiar-estado': function(e, value, row, index) {
            ...
            swal({
                ...
                $.ajax({
                    url: 'inc/botones-data?q=cambiar-estado',
                    type: 'post',
                    data: { id: row.id_boton, estado: status },
                    beforeSend: function() {
                        NProgress.start();
                    },
                    success: function(datos, textStatus, jQxhr) {
                        ...
                        if (n == -1) {
                            ...
                            $('#tabla').bootstrapTable('refresh', { url: 'inc/botones-data?q=ver' });
                        } else {
                            swal.close();
        ...
        'click .editar': function(e, value, row, index) {
            $('#modalLabel').html('Editar Boton');
            $('#formulario').attr('action', 'editar');
            $('#eliminar').show();
            $('#modal_principal').modal('show');

            $("#hidden_id").val(row.id_boton);
            $("#titulo").val(row.titulo);
            $("#url").val(row.url);

            //ELIMINAR
            $('#eliminar').click(function() {
                var nombre = row.titulo;
                swal({
    ...
    $("#tabla").bootstrapTable({
        ...
        columns: [
            [
                { field: 'id_boton', align: 'center', valign: 'middle', title: 'ID', sortable: true, visible: false },
    ...
    $("#formulario").submit(function(e) {
        ...
        $.ajax({
            url: 'inc/botones-data?q=' + $(this).attr("action"),
            dataType: 'html',
            type: 'post',
