...
<body class="<?php include 'menu-class.php';?> fixed-layout">
                        ...
                        <div id="toolbar">
                            <div class='form-inline' role='form'>
                                <div class='form-group'>
                                    <button type='button' class='btn btn-primary' id='agregar' data-toggle='modal' data-target='#modal_principal'>Agregar Autoridad</button>
                                </div>
                            </div>
                        </div>
                        <table id="tabla" data-url="inc/autoridades-data?q=ver" data-toolbar="#toolbar" data-show-export="true" data-search="true" data-show-refresh="true" data-show-toggle="true" data-show-columns="true" data-search-align="right" data-buttons-align="right" data-toolbar-align="left" data-pagination="true" data-side-pagination="server" data-classes="table table-hover table-condensed" data-page-list="[100, 150, 200, 300, 400, All]" data-striped="true" data-icons="icons" data-show-fullscreen="true"></table>
                                                ...
                                                <div class="col-md-12">
                                                    <div class="row">
                                                        <div class="form-group col-md-8 col-sm-12">
                                                            ...
                                                            <input class="form-control input-sm" type="text" name="nombre" id="nombre" autocomplete="off">
                                                        </div>
                                                        <div class="form-group col-md-4 col-sm-6">
                                                            <label for="cargo">Cargo</label>
                                                            <select id="cargo" name="cargo" class="select2" required></select>
                                                        </div>
    ...
    $.ajax({
        ...
        success: function(json) {
            ...
            $.each(json, function(key, value) {
                $('#cargo').append('<option value="' + value.id_autoridad_cargo + '">' + value.cargo + '</option>');
            });
            NProgress.done();
        },
    ...
    window.filaEditar = {
        'click .cambiar-estado': function(e, value, row, index) {
            ...
            swal({
                ...
                text: `¿Actualizar estado de '${row.nombre}' a '${nextStatus}'?`,
                type: "warning",
                ...
                $.ajax({
                    url: 'inc/autoridades-data?q=cambiar-estado',
                    type: 'post',
                    data: { id: row.id_autoridad, estado: status },
                    beforeSend: function() {
                        NProgress.start();
                    },
                    success: function(datos, textStatus, jQxhr) {
                        ...
                        if (n == -1) {
                            ...
                            $('#tabla').bootstrapTable('refresh', { url: 'inc/autoridades-data?q=ver' });
                        } else {
                            swal.close();
    ...
    $("#tabla").bootstrapTable({
        ...
        columns: [
            [
                { field: 'id_autoridad', visible: false },
                { field: 'foto', visible: false },
                { field: 'nombre', align: 'left', valign: 'middle', title: 'Comisión directiva', sortable: true },
                { field: 'cargo', align: 'left', valign: 'middle', title: 'Cargo', sortable: true },
                { field: 'estado', visible: false },
                { field: 'descripcion', align: 'left', valign: 'middle', title: 'Descripción', sortable: true, visible: false },
                ...
                { field: 'id_autoridad_cargo', visible: false }
            ]
        ]
    });
    ...
    function editarFila(e, value, row, index) {
        ...
        $('#modalLabel').html('Editar Autoridad');
        $('#eliminar').show();
        $("#dropurl").val('editar');
        $("#contenDrop").load("./dropzone-autoridades.html");
        $("#estado").prop("disabled", false);
        $('#modal_principal').modal('show');
        $("#hidden_id").val(row.id_autoridad);
        $('#editor').summernote('code', row.descripcion);
        $("#nombre").val(row.nombre);
        $("#cargo").select2('trigger', 'select', {
            data: { id: row.id_autoridad_cargo, text: row.cargo }
        });
        $.ajax({
            url: 'inc/autoridades-data',
            type: 'post',
            data: { q: 'leer_fotos', id: row.id_autoridad },
            dataType: 'json',
    ...
    $('#agregar').click(function() {
        $('#modalLabel').html('Agregar Autoridad');
    ...
    $('#eliminar').click(function() {
        ...
        swal({
            title: "Eliminar",
            text: "¿Desea eliminar la Autoridad: " + nombre + "?",
            type: "warning",
            showCancelButton: true,
            confirmButtonColor: "var(--primary)",
